...
import {
  ...
  query, where, getDocs   // 👈 ADD THESE THREE
...
<style>body{font-family:Arial,sans-serif;padding:32px;color:#111}h1{font-size:22px;margin-bottom:4px}
p{color:#666;font-size:13px;margin-bottom:24px}table{width:100%;border-collapse:collapse;font-size:13px}
...
.present{color:#16a34a;font-weight:700}.absent{color:#dc2626;font-weight:700}</style></head><body>
<h1>AttendQR - Attendance Report</h1><p>Generated: ${new Date().toLocaleString()}</p>
  ...
  return (
    <svg width={100} height={100} style={{transform:"rotate(-90deg)"}}>
      ...
      <circle cx={50} cy={50} r={r} fill="none" stroke={col} strokeWidth={5}
        strokeDasharray={`${(s/total)*circ} ${circ}`}
...
function LoginPage({ onLogin }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const profile = getUserProfile(cred.user);
      if (!profile) { await signOut(auth); setErr("Account not registered in this system."); }
  ...
  const demo = (role) => {
    ...
    setEmail(u.email);
    setPass(role === "teacher" ? "teacher123" : "student123");
  };

  const S = {
    wrap: {minHeight:"100vh",background:"#0a0a0f",display:"flex",alignItems:"center",justifyContent:"center",padding:20},
    card: {width:"100%",maxWidth:400},
    logo: {textAlign:"center",marginBottom:40},
    logoIcon: {width:64,height:64,borderRadius:18,background:"linear-gradient(135deg,#00ff88,#00aacc)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:28,marginBottom:16,boxShadow:"0 0 40px rgba(0,255,136,.35)"},
    logoTitle: {fontFamily:"'Syne',sans-serif",fontSize:26,fontWeight:800,color:"#fff"},
    logoSub: {fontFamily:"'Space Mono',monospace",fontSize:10,color:"#3a3a5a",letterSpacing:3,marginTop:4},
    box: {background:"#0d0d1a",border:"1px solid #1e1e3a",borderRadius:20,padding:32,display:"flex",flexDirection:"column",gap:20},
    label: {fontFamily:"'Space Mono',monospace",fontSize:10,color:"#5a5a7a",letterSpacing:2,display:"block",marginBottom:8},
    input: {width:"100%",padding:"12px 16px",background:"#111128",border:"1px solid #1e1e3a",borderRadius:10,color:"#d0d0e0",fontFamily:"'Space Mono',monospace",fontSize:13,outline:"none",boxSizing:"border-box"},
    err: {padding:"10px 14px",background:"rgba(255,68,68,.1)",border:"1px solid #ff4444",borderRadius:8,color:"#ff6666",fontFamily:"'Space Mono',monospace",fontSize:12},
  };
    ...
    <div style={S.wrap}>
      <div style={S.card}>
        <div style={S.logo}>
          ...
          <div style={S.logoTitle}>AttendQR</div>
          <div style={S.logoSub}>SMART ATTENDANCE SYSTEM</div>
        </div>
        <div style={S.box}>
          ...
          <div>
            ...
            <input value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="you@school.edu" style={S.input}
              onFocus={e=>e.target.style.borderColor="#00ff88"} onBlur={e=>e.target.style.borderColor="#1e1e3a"}/>
          </div>
          <div>
            <label style={S.label}>PASSWORD</label>
            <div style={{position:"relative"}}>
              <input type={showPass?"text":"password"} value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="••••••••"
            ...
            <div style={{display:"flex",gap:10}}>
              {["teacher","student"].map(r=>(
                <button key={r} onClick={()=>demo(r)} style={{flex:1,padding:"9px",borderRadius:8,border:"1px solid #1e1e3a",background:"transparent",color:"#5a5a7a",fontFamily:"'Space Mono',monospace",fontSize:11,cursor:"pointer",letterSpacing:1,transition:"all .2s"}}
                  ...
                  onMouseLeave={e=>{e.target.style.borderColor="#1e1e3a";e.target.style.color="#5a5a7a"}}>
...
function CameraScanner({ onDetected, onClose }) {
  ...
  const runBarcodeDetector = useCallback((det) => {
    const tick = async () => {
      if (doneRef.current) return;
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (!cancelled) { setError("Camera access denied. Use Manual Entry below."); setShowManual(true); }
        return;
      }
      ...
      if ("BarcodeDetector" in window) {
        try {
          ...
          if (!cancelled) { setScanMethod("BarcodeDetector"); setStatus("Point camera at the QR code..."); runBarcodeDetector(det); }
          return;
        } catch(_) {}
      }

      setStatus("Loading QR engine...");
      if (window.jsQR) { if (!cancelled) { setScanMethod("jsQR"); setStatus("Point camera at the QR code..."); runJsQR(); } return; }

      for (const cdn of ["https://unpkg.com/jsqr@1.4.0/dist/jsQR.js","https://cdn.jsdelivr.net/npm/jsqr@1.4.0/dist/jsQR.js"]) {
        try { await tryScript(cdn); if (!cancelled) { setScanMethod("jsQR"); setStatus("Point camera at the QR code..."); runJsQR(); } return; }
        catch(_) {}
      }
      if (!cancelled) { setError("QR engine unavailable. Use Manual Entry below."); setShowManual(true); }
    };

    init();
    return () => { cancelled=true; doneRef.current=true; stopCamera(); };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const submitManual = () => { const t=manualToken.trim(); if(t) handleFound(t); };

  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",zIndex:1000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,overflowY:"auto"}}>
      <div style={{width:"100%",maxWidth:420,background:"#0d0d1a",border:"1px solid #1e1e3a",borderRadius:20,overflow:"hidden"}}>
        <div style={{padding:"16px 20px",borderBottom:"1px solid #1e1e3a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <span style={{fontFamily:"'Space Mono',monospace",fontSize:12,color:"#00ff88",letterSpacing:2}}>QR SCANNER</span>
            {scanMethod && <span style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"#3a3a5a",marginLeft:10}}>via {scanMethod}</span>}
          </div>
          <button onClick={()=>{stopCamera();onClose();}} style={{background:"none",border:"none",color:"#5a5a7a",cursor:"pointer",fontSize:18}}>X</button>
        </div>
        {!error && (
                ...
                <div style={{width:190,height:190,position:"relative"}}>
                  {[{top:0,left:0},{top:0,right:0},{bottom:0,left:0},{bottom:0,right:0}].map((pos,i)=>(
                    <div key={i} style={{position:"absolute",width:28,height:28,...pos,
                      borderTop:("top" in pos)?"3px solid #00ff88":"none",borderBottom:("bottom" in pos)?"3px solid #00ff88":"none",
                      borderLeft:("left" in pos)?"3px solid #00ff88":"none",borderRight:("right" in pos)?"3px solid #00ff88":"none"}}/>
                  ))}
                  <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,#00ff88,transparent)",animation:"scanLine 1.4s ease-in-out infinite"}}/>
                </div>
              </div>
            )}
            {detected && (
              <div style={{position:"absolute",inset:0,background:"rgba(0,255,136,.2)",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:48,color:"#00ff88"}}>OK</div>
              </div>
            )}
          </div>
        )}
        <div style={{padding:"14px 20px",textAlign:"center",borderBottom:"1px solid #111128"}}>
          {error
            ? <div style={{color:"#ff8866",fontFamily:"'Space Mono',monospace",fontSize:12}}>{error}</div>
            : <div style={{color:detected?"#00ff88":"#7a7a9a",fontFamily:"'Space Mono',monospace",fontSize:11,letterSpacing:1}}>{status}</div>
          }
        </div>
        <div style={{padding:"14px 20px 20px"}}>
          <button onClick={()=>setShowManual(!showManual)} style={{width:"100%",padding:"10px",borderRadius:8,border:"1px dashed #2a2a4a",background:"transparent",color:"#4a4a7a",fontFamily:"'Space Mono',monospace",fontSize:11,cursor:"pointer",letterSpacing:1,marginBottom:showManual?12:0}}>
            {showManual?"HIDE MANUAL ENTRY":"ENTER TOKEN MANUALLY"}
          </button>
          {showManual && (
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"#5a5a7a"}}>Copy the token shown under the teacher's QR code:</div>
              <input value={manualToken} onChange={e=>setManualToken(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submitManual()} placeholder="ATT-CS301-..."
                style={{width:"100%",padding:"11px 14px",background:"#111128",border:"1px solid #1e1e3a",borderRadius:9,color:"#d0d0e0",fontFamily:"'Space Mono',monospace",fontSize:12,outline:"none",boxSizing:"border-box"}}
                onFocus={e=>e.target.style.borderColor="#00ff88"} onBlur={e=>e.target.style.borderColor="#1e1e3a"}/>
              <button onClick={submitManual} style={{padding:"11px",borderRadius:9,border:"none",cursor:"pointer",background:"linear-gradient(135deg,#00ff88,#00ccaa)",color:"#0a0a0f",fontFamily:"'Space Mono',monospace",fontSize:12,fontWeight:700,letterSpacing:2}}>
                SUBMIT TOKEN
              </button>
            </div>
          )}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      <div style={{background:"#0d0d1a",border:"1px solid #1e1e3a",borderRadius:20,padding:28,display:"flex",flexDirection:"column",alignItems:"center",gap:18}}>
        ...
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"#2a2a4a",wordBreak:"break-all",textAlign:"center",maxWidth:260,userSelect:"all",cursor:"text"}}>{session.token}</div>
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"#3a3a5a",textAlign:"center"}}>Students can copy token if camera fails</div>
      </div>
        ...
        {(session.scanned||[]).length === 0
          ...
          : (session.scanned||[]).map(s=>(
            <div key={s.studentId} style={{padding:"11px 18px",borderBottom:"1px solid #111128",display:"flex",alignItems:"center",gap:12,animation:"slideIn .3s ease"}}>
              ...
              <span style={{marginLeft:"auto",color:"#00ff88",fontSize:13,fontFamily:"'Space Mono',monospace"}}>OK</span>
            </div>
          ))
        }
      </div>
...
const handleDetected = useCallback(async (token) => {
  ...
  try {
    // 🔍 Query Firestore directly instead of using local sessions
    const q = query(
    ...
    if (snapshot.empty) {
      setScanResult({ ok:false, msg:"No matching active session found." });
      return;
    }
    ...
    if ((session.scanned || []).find(x => x.studentId === user.studentId)) {
      setScanResult({ ok:false, msg:"You are already checked in!" });
      return;
    }
    ...
    setScanResult({ ok:true, msg:"Attendance marked successfully!" });

  } catch (e) {
    console.error(e);
    setScanResult({ ok:false, msg:"Error verifying session." });
  }

}, [user]);

  const simulateScan = () => {
    setSimming(true); setScanResult(null);
    setTimeout(() => {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:22}}>
      ...
      {/* Scanner */}
      <div style={{background:"#0d0d1a",border:`1px solid ${simming?"#00ff88":"#1e1e3a"}`,borderRadius:20,padding:28,display:"flex",flexDirection:"column",alignItems:"center",gap:18,transition:"border .3s"}}>
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"#5a5a7a",letterSpacing:2}}>MARK YOUR ATTENDANCE</div>
        <div style={{width:130,height:130,borderRadius:14,border:`2px solid ${simming?"#00ff88":"#1e1e3a"}`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",transition:"border .3s"}}>
          {simming
            ? <><div style={{position:"absolute",top:0,left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,#00ff88,transparent)",animation:"scanLine 1s ease-in-out infinite"}}/><div style={{fontFamily:"'Space Mono',monospace",fontSize:11,color:"#00ff88"}}>SCANNING...</div></>
            : <div style={{textAlign:"center"}}><div style={{fontFamily:"'Space Mono',monospace",fontSize:32,color:"#2a2a4a",marginBottom:6}}>QR</div><div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"#2a2a4a",letterSpacing:1}}>POINT AT CODE</div></div>
          }
        </div>
        {scanResult && (
          <div style={{padding:"12px 22px",borderRadius:10,background:scanResult.ok?"rgba(0,255,136,.1)":"rgba(255,68,68,.1)",border:`1px solid ${scanResult.ok?"#00ff88":"#ff4444"}`,color:scanResult.ok?"#00ff88":"#ff6666",fontFamily:"'Space Mono',monospace",fontSize:12,fontWeight:700,textAlign:"center",width:"100%",boxSizing:"border-box"}}>
            {scanResult.msg}
          </div>
        )}
        <div style={{display:"flex",gap:10,width:"100%"}}>
          <button onClick={()=>{setScanResult(null);setShowCam(true);}} style={{flex:1,padding:"13px",borderRadius:10,border:"1px solid #1e1e3a",background:"#111128",color:"#7a7aba",fontFamily:"'Space Mono',monospace",fontSize:11,cursor:"pointer",letterSpacing:1,transition:"all .2s"}}
            onMouseEnter={e=>{e.target.style.borderColor="#00aacc";e.target.style.color="#00aacc"}}
            onMouseLeave={e=>{e.target.style.borderColor="#1e1e3a";e.target.style.color="#7a7aba"}}>
            CAMERA
          </button>
          <button onClick={simulateScan} disabled={simming} style={{flex:1,padding:"13px",borderRadius:10,border:"none",cursor:simming?"not-allowed":"pointer",background:simming?"rgba(0,255,136,.15)":"linear-gradient(135deg,#00ff88,#00ccaa)",color:simming?"#00ff88":"#0a0a0f",fontFamily:"'Space Mono',monospace",fontSize:11,fontWeight:700,letterSpacing:1,transition:"all .2s"}}>
            {simming?"SCANNING...":"SIMULATE"}
          </button>
        </div>
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"#2a2a4a",textAlign:"center",letterSpacing:1}}>
          Camera: real QR scan | Simulate: auto-detect open session
        </div>
      </div>
      ...
      {attended.length > 0 && (
        <div style={{background:"#0d0d1a",border:"1px solid #1e1e3a",borderRadius:14,overflow:"hidden"}}>
          <div style={{padding:"12px 18px",borderBottom:"1px solid #1e1e3a"}}>
            <span style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"#5a5a7a",letterSpacing:2}}>ATTENDED TODAY</span>
          </div>
          {attended.map(cls=>(
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div style={{display:"flex",gap:10}}>
        {[{id:"csv",label:"EXPORT CSV",col:"#00aaff"},{id:"pdf",label:"EXPORT PDF",col:"#ff8800"}].map(b=>(
          <button key={b.id} onClick={()=>doExport(b.id)} disabled={!!exporting} style={{flex:1,padding:"12px",borderRadius:10,border:`1px solid ${b.col}`,background:`rgba(${b.id==="csv"?"0,170,255":"255,136,0"},.08)`,color:b.col,fontFamily:"'Space Mono',monospace",fontSize:11,fontWeight:700,cursor:"pointer",letterSpacing:1,opacity:exporting===b.id?.6:1}}>
      ...
      <div style={{background:"#0d0d1a",border:"1px solid #1e1e3a",borderRadius:14,overflow:"hidden"}}>
        <div style={{padding:"12px 18px",borderBottom:"1px solid #1e1e3a"}}>
          <span style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"#5a5a7a",letterSpacing:2}}>STUDENT SUMMARY</span>
        </div>
        {rows.map((r,i)=>(
...
export default function App() {
  ...
  const handleLogin  = (profile) => { setUser(profile); setTab(profile.role==="teacher"?"main":"scan"); };
  ...
  const tabs = isTeacher
    ? [{id:"main",label:"SESSIONS"},{id:"reports",label:"REPORTS"}]
    : [{id:"scan",label:"SCAN"},{id:"reports",label:"REPORTS"}];
        ...
        <div style={{width:"100%",maxWidth:520,padding:"24px 20px 0",display:"flex",flexDirection:"column",gap:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:38,height:38,borderRadius:10,background:"linear-gradient(135deg,#00ff88,#00aacc)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:900,color:"#0a0a0f",boxShadow:"0 0 18px rgba(0,255,136,.3)",fontFamily:"'Space Mono',monospace"}}>Q</div>
              <div>
                <div style={{fontSize:16,fontFamily:"'Syne',sans-serif",fontWeight:800,color:"#fff"}}>AttendQR</div>
                <div style={{fontSize:9,color:"#00ff88",fontFamily:"'Space Mono',monospace",letterSpacing:2}}>FIREBASE</div>
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <button onClick={handleLogout} style={{padding:"6px 10px",borderRadius:8,border:"1px solid #1e1e3a",background:"#0d0d1a",color:"#5a5a7a",cursor:"pointer",fontSize:10,fontFamily:"'Space Mono',monospace",letterSpacing:1}}>LOGOUT</button>
            </div>
          </div>
          <div style={{display:"flex",background:"#0d0d1a",borderRadius:12,padding:4,border:"1px solid #1a1a2e"}}>
            {tabs.map(t=>(
              <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,padding:"10px 0",borderRadius:9,border:"none",cursor:"pointer",background:tab===t.id?"#1a1a2e":"transparent",color:tab===t.id?"#00ff88":"#3a3a5a",fontFamily:"'Space Mono',monospace",fontSize:11,fontWeight:tab===t.id?700:400,letterSpacing:1,transition:"all .2s"}}>
                {t.label}
              </button>
            ))}
          </div>
